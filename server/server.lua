...
AddEventHandler('risky_selltonpc:dodeal', function()
	...
	if item1 >= 1 and xPlayer ~= nil then
		...
		TCE('mythic_notify:client:SendAlert', source, { type = 'success', text = 'You sold ' .. item1amount .. ' '.. Risky.I1.Item  ..' for $' .. item1final, length = 4000 })
	...
	elseif item2 > 1 and xPlayer ~= nil then	
		...
		TCE('mythic_notify:client:SendAlert', source, { type = 'success', text = 'You sold ' .. item2amount .. ' '.. Risky.I2.Item  ..' for $' .. item2final, length = 4000 })
	...
	elseif item3 > 1 and xPlayer ~= nil then
		...
		TCE('mythic_notify:client:SendAlert', source, { type = 'success', text = 'You sold ' .. item3amount .. ' '.. Risky.I3.Item  ..' for $' .. item3final, length = 4000 })
	...
	elseif item4 > 1 and xPlayer ~= nil then
		...
		TCE('mythic_notify:client:SendAlert', source, { type = 'success', text = 'You sold ' .. item4amount .. ' '.. Risky.I4.Item  ..' for $' .. item4final, length = 4000 })
	...
	elseif item1 and item2 and item3 and item4 < 1 and xPlayer ~= nil then
		TCE('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Come back when you are ready to become a REAL drug dealer, maybe next actually BRING drugs with you.', length = 4000 })
	end
end)
